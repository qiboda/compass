...
- [ ] 1. 组合模拟纯函数（compass-strategy `src/sepa/backtest.rs`）
...
- [ ] 2. 绩效统计 + 市值前300基准代理纯函数（compass-strategy `src/sepa/backtest.rs`）
...
- [ ] 3. run_backtest 编排 + CSV 序列化（compass-strategy `src/sepa/backtest.rs`）
...
- [ ] 4. Dolt 写回 backtest_result（compass-data `src/backtest.rs`）
...
- [ ] 5. CLI 子命令注册（compass-data `src/main.rs` + `src/backtest.rs`）
