...
fn score_trend(series: &[&CrossSectionBar], rs_pct: f64) -> (f64, Vec<SepaFactor>) {
    ...
    (
        ...
        vec![
            SepaFactor {
                label_key: "sepa.factor.ma_structure",
                score: structure,
                max: 45.0,
                note_key: None,
                note_args: None,
            ...
            SepaFactor {
                label_key: "sepa.factor.price_position",
                score: position,
                max: 20.0,
                note_key: Some("sepa.note.drawdown"),
                note_args: Some(vec![drawdown]),
            },
            SepaFactor {
                label_key: "sepa.factor.relative_strength",
                score: rs,
                max: 35.0,
                note_key: Some("sepa.note.momentum_percentile"),
                note_args: Some(vec![rs_pct * 100.0]),
            },
        ],
    )
}
...
fn score_theme(best: Option<&ThemeComponents>) -> (f64, Vec<SepaFactor>) {
    let Some(c) = best else {
        return (
            ...
            vec![
                SepaFactor {
                    label_key: "sepa.factor.sector_gain",
                    score: 0.0,
                    max: 30.0,
                    note_key: Some("sepa.note.no_sector_data"),
                    note_args: None,
                },
                SepaFactor {
                    label_key: "sepa.factor.sector_amount",
                    score: 0.0,
                    max: 30.0,
                    note_key: None,
                    note_args: None,
                ...
                SepaFactor {
                    label_key: "sepa.factor.sector_diffusion",
                    score: 0.0,
                    max: 20.0,
                    note_key: None,
                    note_args: None,
                ...
                SepaFactor {
                    label_key: "sepa.factor.news_heat",
                    score: 0.0,
                    max: NEWS_SCORE_MAX,
                    note_key: Some("sepa.note.news_v1"),
                    note_args: None,
                },
            ],
        );
    ...
    (
        ...
        vec![
            ...
            SepaFactor {
                ...
                max: NEWS_SCORE_MAX,
                note_key: Some("sepa.note.news_default"),
                note_args: None,
            },
        ],
    )
...
fn score_capital(series: &[&CrossSectionBar], inputs: &CapitalInputs) -> (f64, Vec<SepaFactor>) {
    ...
    (
        ...
        vec![
            SepaFactor {
                label_key: "sepa.factor.volume_price",
                score: volume_price,
                max: 40.0,
                note_key: None,
                note_args: None,
            ...
            SepaFactor {
                label_key: "sepa.factor.chip_concentration",
                score: chip,
                max: 30.0,
                note_key: None,
                note_args: None,
            ...
            SepaFactor {
                label_key: "sepa.factor.big_capital_inflow",
                score: big_capital,
                max: 30.0,
...
fn score_pattern(series: &[&CrossSectionBar], thermometer_score: f64) -> (f64, Vec<SepaFactor>) {
    ...
    (
        ...
        vec![
            ...
            SepaFactor {
                label_key: "sepa.factor.breakout_confirm",
                score: breakout,
                max: 5.0,
                note_key: Some("sepa.note.thermometer"),
                note_args: Some(vec![thermometer_score]),
            },
        ],
    )
}
...
fn score_risk(series: &[&CrossSectionBar]) -> (f64, Vec<SepaFactor>) {
    ...
    (
        ...
        vec![
            SepaFactor {
                label_key: "sepa.factor.vol_penalty",
                score: -atr_deduction,
            ...
            SepaFactor {
                label_key: "sepa.factor.deep_drawdown",
