...
> **覆盖率门禁**在 CI 执行（coverage job 强制 Rust workspace ≥93% + per-crate 阈值——纯逻辑/serde crate 95%、GUI 主程序 compass 90%；Python 已随 epic #310 退役），太慢不适合 pre-push 本地检查。见 `.dsh/kb/dev/testing.md` 覆盖率章节。
...
（`*.rs`、`*.py`、`*.yml`、`*.md`、CI 模板、hook、脚本），不能只搜
...
**grep 的路径范围必须覆盖全仓所有目录（含隐藏目录与被迁移目录本身），
不预设排除**——`grep -rl` 的路径参数漏掉某个目录 = 该目录内的引用全部漏改
（ref #264 教训：替换列表漏掉技能目录 `.dsh/skills/`，product skill 两处
`.omo/plans/` 残留到 review 才被发现）。替换完成后对**所有已编辑文件**再跑
一次全量残留扫描，不信任"规则集已覆盖"。
