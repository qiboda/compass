...
- **映射表**：`crates/compass-collectors/data/name_en_mapping.csv`（`section,key,value` 三节：
  index 按 symbol / industry 按行业中文；**concept 节已随 issue #283 移除**），随仓库提交；
...
| SEPA 行业 | `SepaRow.industry/industry_en` | industry 经 `display_name`（issue #283 D5：题材评分按 `stock_basic.industry` 分组，**无概念主题**） |
...
- **指数/行业两节**：映射键 = index 按 symbol / industry 按行业中文；
  概念名映射（原 D1-A）已随 issue #283 移除——概念板块不再采集/展示，
  `SharedState.concept_names` 已删（`i18n_name.rs` 头注释明确 concept names
  不在数据层）
- **主题名**：SEPA themes 现为**行业板块**（`stock_basic.industry` 分组，
  issue #283 D5），经 `industry_en` 映射翻译；无概念主题
- 代码、日期值、数值永不翻译

### 覆盖率现状（B1 真实数据验证，concept 节移除后）
...
| 概念名映射（D1-A） | concept_member 加列 / **GUI 层概念名映射** | ~~GUI 层映射（index_basic.name_en 的 concept 行构建）~~ **已撤销**：概念板块随 issue #283 整体移除（采集/展示/映射全链路），`SharedState.concept_names` 已删 | （原决策时点）不动 concept_member schema；概念名直译数据复用 index_basic | 加列扩 schema 面，超出已锁决策 |
...
| index_type 取值 | 含 concept / **仅 official/industry** | 仅 `official`/`industry` 两值（实测 30/90，issue #283 移除 concept 后） | 概念板块废弃后数据层仅两类标的；GUI/Snapshot/搜索路由无需 concept 分支 | 保留 concept 值需维护已删除的映射与展示链路 |
