...
- **Parquet 布局**：
  - `index_daily.parquet`：`symbol, index_type, tradedate, open, high, low, close, volume, amount, adjclose`
    ...
    **`adjclose = close` 占位**（指数无复权概念，东财 `fqt=0` 不复权拉取；占位列使
    `DuckDbProvider` 既有 7 列查询 / 复权缩放（ratio=1.0 → qfq/hfq/none 三档恒等）/
    1w·1M 聚合零改动复用）。
...
### 复权方式三态（ref #345，取代 ref #176 前复权）

`fetch_bars()` 三条路径（1d 内存表 / parquet 回退 / 1w·1M 聚合）均支持**三档复权**，
由 `adjust` 参数（`"qfq"/"hfq"/"none"`，未知值回退 `"qfq"`）选择；`open/high/low/close`
按 `factor_i` 缩放后写入 `Bar`（volume 原样）：

- **qfq 前复权**（默认，修正 ref #176 假设）：`factor_i = ratio_i / r_anchor`，
  `ratio_i = adjclose_i / close_i`，`r_anchor` = 序列中**最后一个 ratio 有效行**的
  ratio——最新 bar=现价。根因：Dolt `adjclose` 实为**后复权**口径（Tushare 源，
  上市首日 ratio=1.0 锚点，如 SZ002832 单调递增至 ≈6.0123），ref #176 直接令
  factor=ratio 会把前复权显示成后复权价（SEPA 面板 25.11 vs 图表 150.97）。
- **hfq 后复权**：`factor_i = ratio_i`（adjclose 本身），无效行 factor=1.0。
- **none 不复权**：`factor_i = 1.0`。
- **无效 ratio**：close≤0 或 adjclose NULL/非有限/≤0 → factor=1.0，**且不参与锚点
  计算**（尾部 NULL adjclose——如 SZ300683 2202 行——不会capture 锚点）。
- **1w/1M 先缩放后聚合**：内层 SELECT 按日 factor 缩放 OHLC，外层再
  `FIRST(open)/MAX(high)/MIN(low)/LAST(close)/SUM(volume)`——保证除权日的
  周/月线高低点准确（聚合后再缩放会失真）。qfq 的 `r_anchor` 在日线层（查询窗口
  内）通过 anchor CTE 计算传入 SQL；聚合序列最后一根有效 bar 的 close 不变。
- 指标（MA/BOLL）在缩放后的 adjusted 序列上实时计算；渲染层无感知。
- 指数（adjclose==close 占位，ratio=1.0）三档结果恒等；SEPA 面板/选股器/回测
  不经过 `fetch_bars`，不受影响（SEPA「最新价」保持 close 现实价）。
...
| fetch_bars 复权三态（ref #345，取代 #176 前复权） | 保持固定前复权 / 三档参数化 | `adjust` 参数三档：qfq=`ratio_i/r_anchor`（锚点=最后有效 ratio 行）、hfq=`ratio_i`、none=1.0；未知值回退 qfq | 修复 Dolt adjclose 后复权口径（ref #176 假设前复权致 SZ002832 显示 6 倍价）；前复权锚点=最后**有效** ratio（尾部 NULL 不 capture）；1w/1M 先缩放后聚合 + 日线层 anchor CTE；SEPA 面板 close 现实价不受影响 | 固定前复权无法表达用户后复权/不复权需求且口径错；GUI 侧缩放需多调用方重复逻辑（同 #176 排除项） |
...
| index_daily 纳入每日 SEPA 管线（ref #303） | 继续只靠手动跑 collectors/index_daily 或独立 sync / 纳入 `scripts/update-database.sh` 标准 5 源流程 | 纳入 `scripts/update-database.sh`：step2 fetch+import、step3 `COLLECTOR_TABLES` allowlist、step4 per-table 增量锚点与导入列表均含 `index_daily`；`index_basic` 作为 `import index_daily` 的伴生表一并加入 allowlist/step4 全量覆盖导出 | 用户 2026-08-25 观察到指数数据停留在旧交易日；“更新数据库”只导 Parquet 不跑采集器时 `index_daily` 会滞后；每日流程已含其余 4 个 SEPA 时序源，纳入后保持单入口、增量锚点一致；`import index_daily` 同时写 `index_basic`，若不纳入会导致 Dolt 脏工作区/名称表 Parquet 过期 | 单独维护 `index_daily` 流程造成两套刷新入口、易漏跑 |
...
| capital_main_flow 数据源切换（issue #339） | 东财 push2 当日全市场截面 + push2his fflow/daykline 历史回补 / **新浪 lscjfb 逐股拉取** | `MoneyFlow.ssl_qsfx_lscjfb?page=1&num=N&sort=opendate&asc=0&daima=sh600519` 逐股（symbol 转小写即 daima，SH/SZ/BJ 前缀保留）；每日窗口 num=20 只导入 `trade_date > last_report_date` 行；历史回补 num=1000 按 [start,end] 过滤；字段映射 `main_net_inflow=r0_net+r1_net`（主力净额）、`main_net_inflow_rate=(r0_net+r1_net)/(r0+r1+r2+r3)×100`（百分数，与东财 f184 历史行口径一致；除零→0；不用 ratioamount——其口径是全市场净额占比）、r0_net/r1_net/r2_net/r3_net → super_large/large/medium/small、`trade_date=opendate`、`update_date=今天`；~5905 股 × 100ms 节流 ≈10-15 分钟；**限制**：逐股窗口 fetch 失败的股被跳过（run() 聚合计数 + WARNING），其当日行不在 auto-heal 的日期级检测内，需手动 `main-flow-backfill --symbols SH600519` 修复；采集目标按 `stock_basic` list/delist 活跃区间过滤（issue #348）：每日路径与显式 `--symbols`/全量回补均只请求目标区间内的上市股（退市股不再产生无效请求；`--symbols` 被过滤清空时报含窗口与数量的错误），导入另加 `main_net_inflow IS NOT NULL` 守卫阻断 NULL 行写入 | 东财 push2 截面无历史、push2his fflow 限流易断且 BJ 错映射为 0 市场（新浪对应 bj830001 类也无覆盖显式跳过）；新浪单股窗口天然兼容停牌（不产生旧日期行）；主力占比口径与东财 f184 语义一致 | 继续 push2 无法自愈断档；fflow 历史 API 对北交所覆盖不全且字段序易错 |
| 日频表空增量日历判定（issue #338） | 0 行即失败 / **日历判定：无交易日则 no-op** | sync 内 4 张日频表（dragon_list/block_trade/capital_main_flow/index_daily）import 后 0 行时查 `ts_trade_day_calendar`（exchange='SSE' is_open=1）窗口 [anchor+1, today]：无交易日/日历不可用（EmptyCalendar）/窗口倒置 → 成功 no-op；有交易日却 0 行 → 失败（消息含 `import returned 0 rows`）；其余错误（MissingRepo/Dolt 等）传播；backfill 路径保留 require_nonzero（0 行=数据源坏了，必须失败） | 非交易日运行 cron 不应误报失败（#306 实证：2026-08-29 周六 COMPASS_AUTO_HEAL=0 报 `dragon_list import returned 0 rows`）；日历覆盖到 2026-12-31 可用 | 一律 no-op 会掩盖真实断源；一律失败误伤周末/节假日 |
