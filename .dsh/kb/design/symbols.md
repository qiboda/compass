...
概念/行业板块使用**独立前缀命名空间** `BK` + 4 位数字（保留东财原始 BK 代码，
采集器零转换）：

- 规范形式：`BK0475`（Dolt-native，无 dot）或 `bk.0475`（dot 形式，规范化为 `BK0475`）；
  解析规则与 SH/SZ/BJ 一致（`parse_explicit_prefix("BK0475")` → `("BK", "0475")`）
- **4 位数字**是硬约束：`validate_symbol` 只接受 `BK` + 恰好 4 位 ASCII 数字
  （`BK047` / `BK04755` / `BKAB12` 一律拒绝）——校验防线与 SH/SZ/BJ 同强度，不因
  新增命名空间削弱注入防护
- `infer_exchange_prefix` 对 `BK` 天然返回 `None`（4 位非 6 位数字，不落入裸码启发式）；
  `exchange_of_symbol("BK0475")` → `"BK"`（原样返回，驱动 GUI 交易所 Tag 显示）
- 官方指数仍复用 `SH`/`SZ` + 6 位（`SH000001` 上证指数、`SZ399001` 深证成指）——与股票
  同格式，000xxx（沪）/399xxx（深）代码段与股票天然不重叠，无歧义；`index_type` 列
  （`official`/`concept`/`industry`）在数据层区分三类标的
