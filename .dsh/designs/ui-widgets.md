# compass 组件使用规范（统一设计语言）— 初稿

> **归档说明**：本文是 ui-designer 产出的**过程归档**，供主 agent 审阅 +
> 用户确认。确认后由主 agent 将最终设计要点同步到 `kb/design/ui-widgets.md`
> （权威文档），一切组件使用规范以 kb/ 版本为准。本文不具权威性。
...
> 原 5 项已按推荐方向解决，删除：①业务层 4 个 citizen 维持「组合关系说明」、
> 不套 8 字段模板；②「未见约束」标注方式接受；③存量直写 `RichText` 不建迁移
> issue（仅新代码收敛到 Label）；④Badge 保持「待接入」不强行落地；⑤示例不标注
> 来源行号。
>
> 本节替换为本次「设计意图 vs 代码现状」审查（用户修正：规范应规定**应然**，
> 不得固化代码**实然**）发现的偏差条目——均已按设计意图改写文档并加 ⚠️ 标注，
> 偏差作为潜在 bug 跟踪，是否全部转 issue 待用户裁决：
...
4. **偏差 #4（权宜实现）** — IconButton 默认尺寸硬编码 `32.0`
   ...
   文档声称「与 control_sm/control_md token 对齐」仅部分成立。→ **转 issue 候选**：
...
5. **偏差 #5（缺失能力）** — Badge 设计规格「min-width 16px」（gui-upgrade.md
   ...
   与实现不符（组件未设任何 Sense）。→ **转 issue 候选**：实现 min-width 16px；
   Badge 目前待接入，影响低。
6. **偏差 #6（权宜实现）** — Dropdown 弹层搜索框用原生 `TextEdit`
   （dropdown.rs:107），绕过 `Input` 组件的统一外观/focus 描边约定（Input 适用场景
   应为「一切自由文本输入」）。→ **转 issue 候选**：复用 `Input` 组件；searchable
   路径当前仅测试覆盖，影响低。

> 以上 #4/#5/#6 属「代码现状 ≠ 设计意图」的潜在 bug，由主 agent 决定转 issue
> 或由实现 agent 顺带修复；#1/#2/#3 为文档自身修正，无代码改动。
