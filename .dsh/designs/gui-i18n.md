...
- **数据名翻译（epic #266 修订）**：数据层携带英文名（`name_en` / `industry_en`
  列 + collectors 静态映射表），GUI 按当前语言取用——有合适翻译就翻译（中↔英
  双向）；**股票名不纳入**（无可靠英文名源，显示回退中文，D0-B）；未收录 →
  NULL → 回退中文，按需增量。代码、日期值、数值仍不翻译。
...
| 数据串（指数/行业/概念/题材名） | 不翻译 / **数据层英文列 + 按语言取用（epic #266）** | 数据层英文列 + 按语言取用 | 语言切换后数据名随界面语言（中↔英双向）；映射表静态维护、未收录回退中文、按需增量；搜索三路匹配 code+name+name_en | 不翻译则英文界面仍显中文；全量翻译静态键不可行（数据量大且动态） |
| 股票名 | 纳入 / **不纳入（D0-B）** | 不纳入 | 无可靠英文名源（官方源仅中文）；显示回退中文；搜索维持 code+name 两路 | 纳入需编造/收集 6000+ 股票英文名，成本高且易错 |
