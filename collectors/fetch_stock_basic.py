    ...
    for attempt in range(EM_MAX_RETRIES):
        try:
            ...
            if attempt < EM_MAX_RETRIES - 1:
                print(f"  Retry {attempt + 1}/{EM_MAX_RETRIES} in {wait:.1f}s: {e}",
                      file=sys.stderr)
                await asyncio.sleep(wait)
            else:
                raise
...
async def main():
    ...
    parser.add_argument(
        ...
        help="Resume from existing CSV instead of overwriting"
    )
    ...
    async with AsyncSession(impersonate="chrome142") as session:
        ...
        params1 = {
            ...
            "fid": "f3", "fs": EM_FS, "fields": EM_FIELDS,
            "ut": "bd1d9ddb04089700cf9c27f6f7426281",
        }
        ...
        total_pages = min(math.ceil(total_count / page_size) if total_count else args.max_pages, args.max_pages)
        ...
        with open(output_path, file_mode, newline="", encoding="utf-8-sig") as f:
            for page in range(start_page, total_pages + 1):
                ...
                if writer is None:
                    api_fields = sorted([k for k in items[0].keys() if k not in ("symbol", "ts_code")],
                ...
                print(f"  Page {page}/{total_pages} | {total} stashed | {progress:.0f}%",
                      file=sys.stderr)
