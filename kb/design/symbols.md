...
| 旧配置自动迁移（D10，issue #181） | 拒绝旧裸码配置启动报错 / 加载时自动迁移 | 加载 config 时检测裸码 default_symbol/watchlist，按旧启发式补前缀（6→SH、8/92→BJ、其余→SZ；dot 形式规范化）并回写文件；回写失败仅 warn 不阻断 | 旧配置文件（如 `default_symbol = "600519"`）无需用户手工改，升级平滑；内存默认值不迁移避免二次迁移 | 拒绝旧配置强制用户手工编辑，破坏向后兼容 |
