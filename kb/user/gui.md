...
- 结果表格 6 列：代码 / 名称 / 最新价 / 20日涨跌幅 / 市值(亿) / 行业。
  默认按市值降序，点击表头切换列与升降序（内存排序）。市值缺失（total_share
  为 NULL）显示 `—`。
- 最多显示 100 行；超过时显示"共 N 只，已显示前 100"。
- 点击任意结果行 → 图表立即切换到该股票（K 线加载），工具栏股票选择器同步。
- 空条件 = 全市场非退市股票按市值降序前 100。
...
吐司垂直堆叠；最多同时显示 10 条，超出时最旧的先消失。点击关闭为预留能力（当前未绑定交互）。
