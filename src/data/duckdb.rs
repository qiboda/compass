...
impl DuckDbProvider {
    ...
    pub async fn save_adj_factors(
        ...
        tokio::task::spawn_blocking(move || -> Result<(), DataError> {
            ...
            let mut stmt = conn
                .prepare(
                    "INSERT OR REPLACE INTO stock_adj_factor (symbol, trade_date, adj_factor) VALUES (?, ?, ?)",
                )
                .map_err(DataError::Database)?;
    ...
    pub async fn upsert_stock_basic(&self, info: &StockBasic) -> Result<(), DataError> {
        ...
        tokio::task::spawn_blocking(move || {
            let conn = conn
                ...
                .map_err(|e| DataError::Parse(format!("mutex poisoned: {e}")))?;
            conn.execute(
                "INSERT OR REPLACE INTO stock_basic (symbol, name, area, industry, market, exchange, list_date, delist_date)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    pub async fn save_limits(
        ...
        tokio::task::spawn_blocking(move || -> Result<(), DataError> {
            ...
            let mut stmt = conn
                .prepare(
                    "INSERT OR REPLACE INTO stock_limit (symbol, trade_date, up_limit, down_limit) VALUES (?, ?, ?, ?)",
                )
                .map_err(DataError::Database)?;
