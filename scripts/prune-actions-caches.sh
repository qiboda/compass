...
# "group"（key 第一个 "-" 之前的 token，如 rust / bench，对应 CI job；
# 注意 bench-check job 的真实 key 为 bench-check-bench-check-...，其首 token
# 是 "bench"）保留 created_at 最新的一份，删除其余 —— 即 "cache miss 后提前清理旧 key"。
#
...
# 注意: 本脚本是被 source 的 library + CLI 双用途；顶层不设置任何 shell
#       选项（不向 caller 施加），直接执行时若需严格模式请自行 set -euo pipefail。
...
#   输出: 每个应删除的 cache id 一行（数字升序、去重）；无删除时输出为空、
#         退出码 0。
#   规则:
...
#     - 其余条目 -> 删除
#   错误语义（对抗契约）:
#     - 顶层 JSON 畸形 / 非数组 / 空串或纯空白 -> 非零退出（显式失败，不静默）
#     - 逐条畸形（缺/非数字 id、非正整数 id、null 或缺 ref/key/created_at、
#       非 ISO 时间、非对象元素）  -> 跳过该条目；绝不含任何未知 id，
#                                    绝不使合法最新条目被挤出删除集
# ----------------------------------------------------------------------------
select_deletions() {
    local json="$1"
    ...
    out="$(
        jq -r '
            def ok_entry:
                ...
                elif ($e.created_at | test("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\\.[0-9]+)?Z$") | not) then false
                elif ($e.id | type) != "number" then false
                elif (($e.id | floor) != $e.id) then false
                elif (($e.id >= 1) | not) then false
                else true end;
...
#   分页终止: 空页 或 collected >= total_count（对抗契约 M2/R3：防死循环）。
#   失败语义: list 失败 = 立即非零退出且零 DELETE（未知状态绝不删除）；
#             list 响应缺 total_count 或非数字 = 同样非零退出零 DELETE
#             （绝不静默回退为 0 —— 那会使满页时停在 page 1 少收）；
...
main() {
    ...
    owner="${owner_repo%%/*}"
    repo="${owner_repo#*/}"
    if [ -z "$owner" ] || [ -z "$repo" ] || [ "$owner" = "$owner_repo" ] \
        || [[ "$repo" == */* ]]; then
    ...
    while :; do
        ...
        total="$(jq -r 'if (.total_count | type) == "number" then .total_count else error("total_count missing") end' <<<"$resp" 2>/dev/null)" || {
            echo "prune-actions-caches: malformed list response (total_count missing)" >&2
            exit 1
        }
